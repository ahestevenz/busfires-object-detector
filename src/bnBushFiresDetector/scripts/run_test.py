...
def _main(args):
    ...
    experiment_path = Path(conf['main']['artefacts']+f'/run_{str(conf["test"]["run"])}')
    if not experiment_path.exists():
        logging.error(f'{experiment_path} does not exist. Please check config.json file')
        return -1
...
def main():
    ...
    # Module specific
    argparser = argparse.ArgumentParser(description='Welcome to the Bushfires Detector testing script')
