...
def display_digits_with_boxes(images: List,
    ...
    colors["blue"] = (0, 0, 255)
    legend = f'{list(colors)[0]}: predicted, {list(colors)[1]}: annotated' if (
        len(pred_bboxes) > 0) else f'{list(colors)[0]}: annotated'
    ...
    for i in range(n):
        ...
        img_to_draw = draw_bounding_boxes_on_image_array(image=images[i], boxes=np.asarray(
            bboxes_to_plot), color=[colors["yellow"], colors["blue"]])
        plt.xticks([])
        plt.yticks([])
